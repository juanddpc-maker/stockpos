...
def render():
    ...
    def kpi(d): 
        r = q(f"SELECT COALESCE(SUM(total),0) as t, COUNT(*) as n FROM ventas WHERE estado='Completada' AND DATE(fecha)='{d}'")
    ...
    c1.metric("💰 Ventas Hoy",     f"${t_hoy:,.2f}",
              f"{((t_hoy-t_ayer)/t_ayer*100):+.1f}% vs ayer" if t_ayer else "—")
    c2.metric("🧾 Trans. Hoy",     n_hoy, f"{n_hoy-n_ayer:+d} vs ayer")
    c3.metric("📊 Ticket Prom.",   f"${ticket_hoy:,.2f}",
              f"{((ticket_hoy-ticket_ayer)/ticket_ayer*100):+.1f}%" if ticket_ayer else "—")
    c4.metric("💼 Apartados",      apt['n'])
    ...
    with col_bar:
        ...
        for i in range(13,-1,-1):
            ...
            labels.append(d.strftime("%d/%m")); vals.append(float(r[0]['t']))
        fig = go.Figure(go.Bar(
            x=labels, y=vals,
            marker_color=["#4299e1"]*13+["#f6ad55"],
    ...
    with col_top:
        ...
        if top:
            ...
            st.dataframe(df2[['Producto','und','ing']].rename(columns={'und':'Unidades','ing':'Ingresos'}),
