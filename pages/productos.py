...
def _lista(cats):
    ...
    buscar   = fc1.text_input("🔍 Buscar", placeholder="Nombre del producto...",
    ...
    for p in prods:
        ...
        por_cat.setdefault(cat_key, []).append(p)
    ...
    for cat_label, cat_prods in por_cat.items():
        ...
        for p in cat_prods:
            stock     = int(p['stock'])
            min_row   = q("SELECT COALESCE(MIN(min_stock),5) as m FROM inventario WHERE producto_id=?",
                          (p['id'],))
            min_s     = int(min_row[0]['m']) if min_row else 5
            status    = ("❌ Sin stock"  if stock == 0
                         else "⚠️ Stock bajo" if stock <= min_s
                         else "✅ Normal")
            talla_lbl = TIPO_TALLA_LABELS.get(p['tipo_talla'], '—')

            with st.expander(
                f"{p['emoji']} **{p['nombre']}** · "
                f"${float(p['precio']):,.2f} · {status} · 📏 {talla_lbl}"
            ):
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("Precio",      f"${float(p['precio']):,.2f}")
                m2.metric("Stock total", stock)
                m3.metric("Categoría",   f"{p['cat_emoji'] or ''} {p['cat_nombre'] or '—'}")
                m4.metric("Tipo talla",  talla_lbl)

                # Stock por talla
                if p['tipo_talla'] != 'unico':
                    tallas_stock = q(
                        "SELECT talla, SUM(cantidad) as cant FROM inventario "
                        "WHERE producto_id=? GROUP BY talla ORDER BY cant DESC",
                        (p['id'],)
                    )
                    if tallas_stock:
                        st.markdown("**Stock por talla:**")
                        tcols = st.columns(len(tallas_stock))
                        for i, ts in enumerate(tallas_stock):
                            color = ("🔴" if ts['cant'] == 0
                                     else "🟡" if ts['cant'] <= 3
                                     else "🟢")
                            tcols[i].metric(ts['talla'], f"{color} {ts['cant']}")
    ...
    with st.form("form_nuevo", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 Guardar producto", type="primary", use_container_width=True):
            if not nombre.strip():
                ...
                pid = run(
                    "INSERT INTO productos(nombre,precio,categoria_id,emoji,"
                    "descripcion,codigo_barras,tipo_talla) VALUES(?,?,?,?,?,?,?)",
                    (nombre.strip(), precio, cat_opts[cat_sel], emoji, desc, codigo, tipo_talla)
                )
                for talla in tallas:
                    run(
                        "INSERT INTO inventario(producto_id,talla,localidad,"
                        "cantidad,min_stock,max_stock) VALUES(?,?,?,?,?,?)",
                        (pid, talla, localidad, cant_ini, min_stock, 100)
                    )
                st.success(f"✅ **{nombre}** creado con {len(tallas)} talla(s)")
                st.rerun()


def _form_editar(p, cats):
    cat_opts     = {f"{c['emoji']} {c['nombre']}": c['id'] for c in cats}
    cat_default  = next((k for k, v in cat_opts.items() if v == p['categoria_id']),
                        list(cat_opts.keys())[0])
    tipo_default = next((k for k, v in TIPO_TALLA_OPTS.items() if v == p['tipo_talla']),
                        list(TIPO_TALLA_OPTS.keys())[0])

    with st.form(f"edit_{p['id']}"):
        c1, c2  = st.columns(2)
        nombre  = c1.text_input("Nombre",  value=p['nombre'])
        precio  = c2.number_input("Precio", value=float(p['precio']),
        ...
        cg, ce   = st.columns(2)
        guardar  = cg.form_submit_button("💾 Guardar cambios",  type="primary",
                                          use_container_width=True)
        eliminar = ce.form_submit_button("🗑️ Eliminar producto", use_container_width=True)

        if guardar:
            nuevo_tipo = TIPO_TALLA_OPTS[tipo_sel]
            run(
                "UPDATE productos SET nombre=?,precio=?,categoria_id=?,emoji=?,"
                "descripcion=?,codigo_barras=?,tipo_talla=? WHERE id=?",
            ...
            if nuevo_tipo != p['tipo_talla']:
                ...
                for talla in TALLAS.get(nuevo_tipo, ["Única"]):
                    existe = q("SELECT id FROM inventario WHERE producto_id=? AND talla=?",
                               (p['id'], talla))
                    if not existe:
                        run(
                            "INSERT INTO inventario(producto_id,talla,localidad,"
                            "cantidad,min_stock,max_stock) VALUES(?,?,?,0,2,100)",
