...
def _panel_venta():
    ...
    with col_cart:
        ...
        if not cart:
            ...
            if c2.button("✅ Procesar venta", type="primary", use_container_width=True):
...
def _panel_apartar():
    ...
    if st.session_state.apartado_ok:
        ...
        st.success(f"💼 Apartado **{folio}** registrado — Total: **${total:,.2f}** · Saldo pendiente: **${saldo:,.2f}**")
        if st.button("🔄 Nuevo apartado", type="primary"):
            st.session_state.apartado_ok = None
            st.rerun()
        return
    ...
    with col_cart:
        ...
        if not cart:
            ...
            anticipo = ac1.number_input("💵 Anticipo recibido *", min_value=0.0,
                                        max_value=float(total), step=10.0, format="%.2f",
                                        help="Monto que paga el cliente hoy")
            dias = ac2.number_input("📅 Días para liquidar", min_value=1, value=15)
...
def _get_prods():
    ...
    buscar  = fc1.text_input("🔍 Buscar", placeholder="Nombre...", label_visibility="collapsed", key="pos_buscar")
    cat_sel = fc2.selectbox("Cat.", cat_opts, label_visibility="collapsed", key="pos_cat")
...
def _render_prods(prods, modo):
    ...
    if not prods:
        st.info("No se encontraron productos"); return
    for p in prods:
        stock = int(p['stock']); min_s = int(p['min_stock'])
        ...
        if stock > 0:
            if pc4.button("➕", key=f"add_{modo}_{p['id']}", use_container_width=True):
                cart = st.session_state[cart_key]
                if p['id'] in cart:
                    if cart[p['id']]['qty'] < stock: cart[p['id']]['qty'] += 1
                    else: st.toast("⚠️ Sin más stock")
                else:
                    cart[p['id']] = {'nombre':p['nombre'],'emoji':p['emoji'],
...
def _render_cart(cart, modo):
    ...
    for pid, item in list(cart.items()):
        ...
        if c2.button("−", key=f"dec_{modo}_{pid}", use_container_width=True):
            ...
            st.rerun()
        c3.markdown(f"<div style='text-align:center;padding-top:8px;font-weight:700'>{item['qty']}</div>",
                    unsafe_allow_html=True)
        if c4.button("＋", key=f"inc_{modo}_{pid}", use_container_width=True):
            if cart[pid]['qty'] < item['stock']: cart[pid]['qty'] += 1
            else: st.toast("Sin más stock")
            st.rerun()
